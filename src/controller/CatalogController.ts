  ...
  try {
    ...
    if (authorizationParams.group === ROLE_USER_ALPELOTEO ||
      ...
      if (httpMethod === 'GET' || httpMethod === 'DELETE') {
        ...
        if (httpMethod === 'GET' && pathParameters !== null && !path.includes('parents')) {
        ...
        if (httpMethod === 'GET' && queryStringParameters !== null) {
          ...
          if (path.includes('parents')) {
            const {id} = pathParameters!;
            result = await findByParentId(authorizationParams, parseInt(id!), filters, context);
          } else {
...
export const create = async (authorizationParams: Authorization, body: Catalog, context: Context) => {
  try {
    const catalogService = new CatalogService();
    return await catalogService.create(authorizationParams, body);
  } catch (error: any) {
    console.error('----->', error);
...
export const find = async (authorizationParams: Authorization, searchFilter: SearchFilter, context: Context) => {
  try {
    const catalogService = new CatalogService();
    return await catalogService.find(authorizationParams, searchFilter);
  } catch (error: any) {
    console.error('----->', error);
    error.context = createLogContext(context, error.message);
    throw error;
...
export const findById = async (authorizationParams: Authorization, id: number, context: Context) => {
  try {
    const catalogService = new CatalogService();
    return await catalogService.findById(authorizationParams, id);
  } catch (error: any) {
    console.error('----->', error);
    error.context = createLogContext(context, error.message);
    throw error;
...
export const findByParentId = async (authorizationParams: Authorization, parentId: number,
    searchFilter: SearchFilter, context: Context) => {
  try {
    const catalogService = new CatalogService();
    return await catalogService.findByParentId(authorizationParams, parentId, searchFilter);
  } catch (error: any) {
    console.error('----->', error);
    error.context = createLogContext(context, error.message);
    throw error;
...
export const update = async (authorizationParams: Authorization, body: Catalog, context: Context) => {
  try {
    const catalogService = new CatalogService();
    return await catalogService.update(authorizationParams, body);
  } catch (error: any) {
    console.error('----->', error);
    error.context = createLogContext(context, error.message);
    throw error;
